...
void update_game( player_t *player );
void render();
...
main(int argc, char *argv[])
{
    ...
    while( gameRunning ){
        ...
        while( SDL_GetTicks() > next_game_tick && loops < 4 ){
            update_game( &player );
        ...
        interpolation = (SDL_GetTicks() + 16 - next_game_tick)/(16);
        // Interpolating
        tux.x = player.x * interpolation;
        tux.y = player.y * interpolation;
