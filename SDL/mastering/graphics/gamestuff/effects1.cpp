...
main(int argc, char *argv[])
{
    ...
    //if( alien != NULL ){
        ...
        */
        ...
        //
        SDL_BlitSurface( alien, NULL, screen, NULL );
...
render( float alpha )
{
    ...
    SDL_SetAlpha( alien, SDL_RLEACCEL | SDL_SRCALPHA, i );
    SDL_BlitSurface( alien, NULL, screen, NULL );
}
*/
...
render( float alpha )
{
    SDL_Rect tmp, tmp2;
    static int i = 0;
    static int w = 0, line = 1;

    printf("alpha = %f\n", alpha);
    //i = i + (i + alpha);
    i++;

    tmp.x = SCREEN_WIDTH - w;
    tmp.y = SCREEN_HEIGHT - (40 * line);
    tmp.w = w;
    tmp.h = 40;
    ...
    // clearing old positions
    SDL_FillRect( screen, &tmp, SDL_MapRGB( screen->format, 0x00, 0x00, 0x00 ) );
    //SDL_SetAlpha( alien, SDL_RLEACCEL | SDL_SRCALPHA, i );
    //SDL_BlitSurface( alien, NULL, screen, NULL );
    w++;
}
