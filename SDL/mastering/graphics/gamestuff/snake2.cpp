...
move_objects( )
{
    ...
    for(int i = snake_blocks.size() - 1; i > 0 ;i--){
        snake_blocks[i]->vel.x = snake_blocks[i - 1]->vel.x;
        snake_blocks[i]->vel.y = snake_blocks[i - 1]->vel.y;

        if( !snake_blocks[i]->is_Visible ){
            snake_blocks[i]->pos.x = snake_blocks[i - 1]->pos.x;
            snake_blocks[i]->pos.y = snake_blocks[i - 1]->pos.y;
            snake_blocks[i]->is_Visible = true;
            continue;               
        }

        next_x = snake_blocks[i]->pos.x + snake_blocks[i]->vel.x;
        next_y = snake_blocks[i]->pos.y + snake_blocks[i]->vel.y;
        ...
        snake_blocks[i]->pos.x = next_x;
        snake_blocks[i]->pos.y = next_y;
