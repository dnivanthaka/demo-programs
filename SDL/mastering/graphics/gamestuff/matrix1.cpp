...
render( float alpha )
{
    ...
    //creature.pos.y = creature.pos.y + (creature.vel.y * alpha);
    //creature.pos.y = creature.pos.y + (creature.vel.y );
    ...
    tmp.y = (int)(creature.pos.y + (creature.vel.y * alpha ));
...
handle_clipping()
{
    ...
    */
    ...
    // Tux is moving down
    if( creature.is_Visible && creature.vel.y < 0 && creature.pos.y <= 20 ){
            creature.clip_rect.h += creature.vel.y;
            creature.pos.y = 20;
            printf("XXXXX");
    }else if( creature.is_Visible && creature.vel.y > 0 && (creature.pos.y + creature.dim.h > (SCREEN_HEIGHT - 20)) ){
            creature.clip_rect.h += creature.vel.y;
            creature.pos.y = 20;
            printf("YYYYY");
    }else if( !creature.is_Visible && creature.vel.y > 0 && creature.pos.y <= 20 ){
            creature.clip_rect.h -= creature.vel.y;
            creature.pos.y = 20;
            printf("ZZZZZ");
    }else if( creature.is_Visible && creature.clip_rect.h > 0 ){
            printf("KKKKK");
            if( creature.clip_rect.h - creature.vel.y <= 0  )
                creature.clip_rect.h = 0;
            else
                creature.clip_rect.h -= creature.vel.y;
            //creature.clip_rect.h = 0;
            creature.pos.y = 20;
            printf("creature.clip_rect.h = %d\n", (creature.clip_rect.h % 4));
    }

    if( creature.clip_rect.h < 0 )
        creature.clip_rect.h = 0;
...
init_game()
{
    ...
    creature.clip_rect.h = creature.dim.h;
}
