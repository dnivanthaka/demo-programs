...
// Im passing the object itself for checking, but it needs to iterate all the collidable objects
int
player_can_jump( struct player *d, struct solid_object *ob, int height )
{
    int ret = height;

    if( ob->screen_x <= d->screen_x && (ob->screen_x + ob->w) >= d->screen_x && ((ob->screen_y + ob->h) - height) > 0 ){
            printf("collision\n");
            /*
            if( d->screen_y >= (ob->screen_y + ob->h) ){
                d->screen_y = ob->screen_y + ob->h;
            ...
            */
            printf(" d = %d\n", d->screen_y - height);
            printf(" ob = %d\n", ob->screen_y + ob->h);
            //ret =  (d->screen_y - height) - (ob->screen_y + ob->h);
            ret = 15;
...
void
draw_solid_object( struct solid_object *ob, SDL_Surface *src, SDL_Surface *dest )
{
    int w, h;
    int i, j, times_x, times_y;
    w = ob->w;
    h = ob->h;

    times_x = w / ob->image.w;
    times_y = h / ob->image.h;
    
    for(i=0;i<times_x;i++){
        for(j=0;j<times_y;j++){
            apply_surface( ob->screen_x + (ob->image.w * i), ob->screen_y + (ob->image.h * j), src, dest, &ob->image );
        }
    }
}
