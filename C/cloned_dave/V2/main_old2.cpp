    ...
    */
    ...
    while( quit == false ){
        ...
        if( is_jumping ){
            ...
            reg_y_off = (next_y % 15 == 0) ? next_y : next_y + (next_y % 15);
            reg_y_off = ceil(reg_y_off / 16);

            // test for top walls
            if( ascending && get_obj_type( &lvl, ceil(reg_x_off), ceil(reg_y_off) ) == 01 ){
                printf("Up\n");
                //is_jumping = false;
                initial_jmp_vel = -1 * initial_jmp_vel;
            ...
            }else if( !ascending && get_obj_type( &lvl, ceil(reg_x_off), ceil(reg_y_off) ) == 01 ){
                printf("Down\n");
                printf("x = %d\n", d.screen_x);
                printf("y = %d\n", (int)ceil(next_y / 16));
                printf("obj = %d\n", get_obj_type( &lvl, d.screen_x / 16, next_y / 16 ));
                is_jumping = false;
                //initial_jmp_vel = 18;
            ...
            if( get_obj_type( &lvl, d.screen_x / 16, next_y / 16 ) ){
