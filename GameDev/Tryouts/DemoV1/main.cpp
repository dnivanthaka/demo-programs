...
SDL_Rect bricks_collection[3] = {
    ...
    { TILE_WIDTH, 0, TILE_WIDTH, TILE_WIDTH }, 
    { TILE_WIDTH * 2, 0, TILE_WIDTH, TILE_WIDTH }
};
...
load_resources()
{
    ...
    //background = load_image( "assets/back1.jpg" );
     ...
     background2 = load_image( "assets/sky1.png" );
...
main( int argc, char *argv[] )
{
    ...
    */
    ...
    //----------------------------------------------------------
/*
    background2 = SDL_CreateRGBSurface(SDL_SWSURFACE, player.width, 
            player.height, SCREEN_BPP, screen->format->Rmask, 
            screen->format->Gmask, screen->format->Bmask, 
            screen->format->Amask );
*/
    ...
    */
    ...
    //draw_layout( &screen );
    draw_layout( &bricks, &screen, bricks_collection, scene );
    //SDL_FillRect( background, NULL, SDL_MapRGB( background->format, 0xff, 0x00, 0x00 ) );
    // -------------------------------------------------------------
    ...
    while( gameIsRunning ){
        ...
        while( last_tick < this_tick ){
            ...
            if( player.action == RIGHT && player.vel_x > 0 ){
                ...
                tmp = get_tile_type( tile_x + player.vel_x + player.width, player.y, scene  );
            ...
            if( player.action == LEFT && player.vel_x < 0 ){
                tmp = get_tile_type( tile_x + player.vel_x, player.y, scene );
                if( tmp == 1 || tmp == 2 || tmp == 3 ){
                    player.vel_x = 0;
                }
            }
            ...
            if( player.is_jumping && player.vel_y > 0 ){
                tmp = get_tile_type( tile_x + player.vel_x, player.y, scene );
                tmp2 = get_tile_type( tile_x + player.vel_x + (player.width / 2), player.y, scene );
            ...
            //Bottom
            if( player.is_jumping && player.vel_y < 0 ){
                tmp = get_tile_type( tile_x + player.vel_x, player.y + player.height, scene );
                tmp2 = get_tile_type( tile_x + player.vel_x + player.width + 1, player.y + player.height, scene );
                if( tmp == 1 || tmp2 == 1 || tmp == 2 || tmp2 == 2 || tmp == 3 || tmp2 == 3 ){
            ...
            if( !player.is_jumping && player.vel_y == 0 ){
                if( player.action == RIGHT ){
                    tmp = get_tile_type( tile_x + 2 , player.y + player.vel_y + player.height, scene );
                    tmp2 = get_tile_type( tile_x + (player.width / 2) , player.y + player.vel_y + player.height, scene );
                    if( tmp == 0 && tmp2 == 0 ){
                    ...
                    tmp = get_tile_type( tile_x, player.y + player.vel_y + player.height, scene );
                    tmp2 = get_tile_type( tile_x + (player.width / 2), player.y + player.vel_y + player.height, scene );
                    if( tmp == 0 && tmp2 == 0 ){
      ...
      */      
        ...
        */
        ...
        if( j == screen->w ){
            ...
            background2 = t_s;
            //SDL_FillRect( background, NULL, SDL_MapRGB( background->format, 0x00, 0x00, 0xff ) );
            //draw_layout( &bricks, &background, bricks_collection, scene + 1 );
            scene = (scene + 1) % 2;
        }
        draw_layout( &bricks, &background, bricks_collection, scene );
        draw_layout( &bricks, &background2, bricks_collection, (scene + 1) % 2 );
        // -------------------------------------------
        ...
        tmp.x = 0;
        tmp.y = 0;
        apply_surface( screen->w - j, 0, background2, screen, &tmp );
